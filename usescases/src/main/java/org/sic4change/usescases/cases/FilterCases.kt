...
class FilterCases(private val casesRepository: CasesRepository) {
    ...
    suspend fun invoke(nameSurnames: String, place: String, physic: Boolean?, sexual: Boolean?,
                       psychologic: Boolean?, social: Boolean?, economic: Boolean?, status: String?, rangeData: String) : List<Case> =
        casesRepository.filterCases(nameSurnames, place, physic, sexual, psychologic, social, economic, status, rangeData)
